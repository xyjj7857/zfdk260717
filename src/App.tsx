...
export default function App() {
  ...
  const handleTransfer = async (e: React.FormEvent) => {
    ...
    try {
      const endpoint = transferDirection === 'to_futures' ? '/api/transfer/to-futures' : '/api/transfer/to-spot';
      const res = await fetch(`${endpoint}?accountId=${selectedAccountId}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (data.success) {
        setTransferStatus({ 
          type: 'success', 
          msg: transferDirection === 'to_futures' 
            ? `成功划转 ${amount} USDT 到合约账户` 
            : `成功划转 ${amount} USDT 到现货账户` 
        });
        setTimeout(() => {
    ...
    <Router>
      ...
      {showTransferModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="w-full max-w-sm bg-white rounded-[2.5rem] p-8 shadow-2xl animate-in zoom-in duration-300">
            <div className="flex flex-col items-center text-center mb-8">
              <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-4 ${transferDirection === 'to_futures' ? 'bg-purple-50' : 'bg-emerald-50'}`}>
                <ArrowLeftRight className={transferDirection === 'to_futures' ? 'text-purple-600' : 'text-emerald-600'} size={32} />
              </div>
              <h2 className="text-2xl font-black text-slate-900 tracking-tight">资金划转</h2>
              <p className="text-slate-500 text-sm mt-2 font-medium">
                {transferDirection === 'to_futures' ? '现货账户 -> 合约账户' : '合约账户 -> 现货账户'}
              </p>
            </div>
            <form onSubmit={handleTransfer} className="space-y-4">
              <div className="space-y-1">
                ...
                <input 
                  ...
                  placeholder="0.00"
                  className={`w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 transition-all font-mono ${
                ...
                <div className="flex justify-between px-1 mt-1">
                  <span className="text-[10px] text-slate-400 font-bold">
                    {transferDirection === 'to_futures' 
                      ? `现货余额: ${parseFloat(account?.spotBalance || '0').toFixed(2)} USDT` 
                      : `合约总余额: ${parseFloat(account?.totalBalance || '0').toFixed(2)} USDT`
                    }
                  </span>
                  <button 
                    type="button" 
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <button 
                  ...
                  className={`py-4 text-white rounded-xl font-black uppercase tracking-widest scale-100 active:scale-[0.98] transition-all disabled:opacity-50 shadow-lg ${
                    transferDirection === 'to_futures' 
      ...
      {expanded ? (
        <>
          <div className="flex items-center justify-between mb-0.5">
            ...
            <div className={`w-5 h-5 bg-white rounded-md flex items-center justify-center border border-slate-100 shadow-sm ${
              onClick 
                ? theme === 'emerald' 
