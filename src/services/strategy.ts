...
export class StrategyEngine {
  ...
  private async checkPositionTimeout() {
    ...
    for (let index = 0; index < sortedOpenTrades.length; index++) {
      ...
      if (holdTimeMs >= maxHoldTimeMs) {
        ...
        await this.closeCurrentPosition(symbol);
      }
    }
  }
  ...
  private async closeCurrentPosition(targetSymbol?: string) {
        ...
        for (const [key, config] of Object.entries(conditions) as [string, any][]) {
          ...
          if (key === 'longShort') {
            ...
            if (buyEnabled || sellEnabled) {
               ...
               if (!isPassLocal) {
                  passed = false; failReasonAmp = `多空:${buyRatio.toFixed(2)}`; break;
               }
               ampSide = localSide;
            }
          } else if (key === 'amp') {
            ...
            if (val < min || val > max) { passed = false; failReasonAmp = `AMP:${val.toFixed(2)}`; break; }
          } else if (key === 'm') {
            const val = item.volume15;
            const [min, max] = config.range;
            if (val < min || val > max) { passed = false; failReasonAmp = `M:${(val/1000000).toFixed(2)}`; break; }
          } else if (key === 'fundingRateOptimization') {
            const { enabled, windowMinutes, shortThreshold } = config;
            if (enabled && ampSide === 'SELL') {
    ...
    if (tradableResults.length > 0 && !isManual && this.isRunning && !this.isOrdering && !this.pendingOrderSymbol) {
      if (Date.now() - startTime <= (this.settings.order?.positiveWindow || 2) * 1000) {
        ...
        if (remainingCapacity > 0 && eligibleResults.length > 0) {
          const tradeCount = Math.min(remainingCapacity, eligibleResults.length);
          this.addLog('下单', `检测到当前持仓数量: ${currentActiveCount}, 最大限制: ${maxAllowed}, 将对前 ${tradeCount} 个符合策略（振幅/单K）条件的币对下单`, 'info');
          
          for (let idx = 0; idx < tradeCount; idx++) {
            const coinToTrade = eligibleResults[idx];
            await this.executeTrade(coinToTrade, startTime);
          }
        }
      }
    }
  ...
  async executeTrade(coin: any, scanStartTime: number) {
    ...
    try {
      ...
      this.addLog('下单', `[最高] 正向单下单: ${coin.symbol}, 方向: ${coin.side || 'BUY'}, 数量: ${formattedQuantity}, 价格: ${currentPrice}${isSecondOrder ? ' (已启用第二仓单自定义比例/最大额度限制)' : ''}`, 'success');
      ...
      await this.handleTPAndSL(coin.symbol, formattedQuantity, scanStartTime, symbolInfo, order.orderId.toString(), executionData, coin.side || 'BUY', coin, isSecondOrder);
  ...
  private async handleTPAndSL(symbol: string, quantity: string, scanStartTime: number, symbolInfo: any, orderId: string, executionData?: { avgPrice: number, qty: number }, side: 'BUY' | 'SELL' = 'BUY', scannerData?: any, isSecondOrder: boolean = false) {
    ...
    try {
      ...
      const isSingleK = scannerData?.sourceMode === 'singleK';
      ...
      const stepSize = parseFloat(lotSizeFilter.stepSize);
      const minQty = parseFloat(lotSizeFilter.minQty) || 0;
      const actualQuantity = formatQtyWithStepSize(Math.abs(this.currentPosition.amount), stepSize, minQty);
      ...
      if (!hasAlgo && slEnabled !== false && sideSlEnabled) {
        try {
          // 预检：在下止损单前，先获取最新市场价
          const ticker = await this.binance.getTickerPrice(symbol);
          const currentPrice = parseFloat(ticker.price);
